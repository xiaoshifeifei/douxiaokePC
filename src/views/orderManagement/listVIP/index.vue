    ...
    <el-card>
      <div class="table_top" ref="heightTop">
        <div>
          ...
          <el-select style="margin-left:20px"  v-model="value2" placeholder="订购方式">
            <el-option label="是" value='1'></el-option>
            <el-option label="否" value='0'></el-option>
          </el-select>
          ...
          <el-button type="primary" @click="getList" icon="el-icon-search"
            >查询</el-button
          >
          <el-button type="primary" @click="reaseList" 
            >重置条件</el-button
          >
        </div>
      </div>
      <div class="btnCon">
          <el-button class="btnn" type="primary" @click="addProduct()">导出</el-button>
      ...
      >
        ...
        >
          ...
          <div class="pl_20">
            <el-button class="btn2" type="primary" @click="commitClick">确定</el-button>
      ...
      >
        ...
        >
          ...
          <el-table-column prop="store_name" align="center" label="订单编号" />
          <el-table-column prop="contacts" align="center" label="支付时间" />
          <el-table-column prop="tel" align="center" label="账户手机号" />
          <el-table-column prop="address" align="center" label="门店名称" />
          <el-table-column prop="address" align="center" label="会员类型" />
          <el-table-column prop="sell_name" align="center" label="生效时间" />
          <el-table-column prop="des" align="center" label="支付金额" />
          <el-table-column prop="des" align="center" label="订购方式" />
        </el-table>
      ...
      >
        ...
        >
          ...
          <div class="pl_20">
            <el-button class="btn2" type="primary" @click="commitRemork">保存</el-button>
...
export default {
  ...
  methods: {
    ...
    async handleBatchDownload(selectImgList) {
      ...
      Promise.all(promises).then(() => {
        zip.generateAsync({
          ...
          FileSaver.saveAs(content, "蓝V信息数据.zip") // 利用file-saver保存文件
        })
      })
      console.log('成功啦');
    },
    ...
    addProduct() {
      ...
      if (this.multipleSelection.length == 0 && this.multipleSelectionAll.length == 0) {
        this.page=1
        this.limit = 1000;
        this.getList().then(() => {
      ...
      } else if (this.multipleSelection.length > 1000 || this.multipleSelectionAll.length > 1000) {
        this.$message("导出数据不能超过1000条");
        return;
      } else if (this.multipleSelection.length > 0 || this.multipleSelectionAll.length > 0) {
        const arrayNew =[...new Set(this.multipleSelectionAll.concat(this.multipleSelection))] 
