        ...
        >
          ...
          >
            ...
            <el-table-column
              ...
              label="所属分销员"
            />
            <el-table-column prop="team_name" align="center" label="所属渠道" />
          </el-table>
  ...
  methods: {
    ...
    addProduct() {
      ...
      if (
        ...
        console.log("arrayNew11", arrayNew);
        let ary2 = [];
    ...
    exportExcel(params) {
      ...
      let tittle =
        thirteenBitTimestamp(new Date().getTime()) + "推广数据订单列表.xlsx";
      try {
        FileSaver.saveAs(
          new Blob([wbout], {
    ...
    exportExcel8(params) {
      ...
      // this.$emit('update:addEmpShow1', false)
      let tittle =
        thirteenBitTimestamp(new Date().getTime()) + "渠道商推广用户明细.xlsx";
      try {
        FileSaver.saveAs(
          new Blob([wbout], {
            type: "application/octet-stream"
    ...
    async getList() {
      ...
      let res = await subOrderList(data);
      if (res.code === 0) {
        this.allArray = res.data.rows;
